    ...
    public class ShellViewModel : Conductor<object>
    {
        ...
        */
        ...
        private void loadTaxonomyViewModelObj(string lbl)
        {
            TaxonomyVM = new TaxonomyViewModel();
            //set result type
            TaxonomyVM.ResultQuant = SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.ResultTypes[0];

            //set input params
            TaxonomyInputParam inputParam;

            for (int i = 0; i < SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.Parameters.Count(); i++)
            {
                string param = SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.Parameters[i].name;
                string quantity = SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.Parameters[i].Quantity.name;
                string optional = SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.Parameters[i].optional.ToString();
            ...
            /*
            //set external URL
            if (SampleSOA.CapabilityScope.Activities[0].Taxons[0].Uri != null)
            {
                TaxonomyVM.ExternalURL = SampleSOA.CapabilityScope.Activities[0].Taxons[0].Uri;
            }
            */

            //set documentation
            if (SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.Definition != null)
            {
                TaxonomyVM.EmbeddedDoc = SampleSOA.CapabilityScope.Activities[0].Taxons[0].Taxon.Definition.Value.ToString();
            }
        ...
        private void loadTechniqueViewModelObj(string lbl)
        {
            ...
            TechniqueVM.TaxonomyName = SampleSOA.CapabilityScope.Activities[0].Techniques[0].Technique.TaxonName;
        ...
        public void OpenXMLFile()
        {
            ...
            */
            ...
            for (int processTypeIndex = 0; processTypeIndex < SampleSOA.CapabilityScope.Activities[0].Taxons.Count(); processTypeIndex++)
            {
                //add nodes in the Taxonomy level to the tree
                string taxonomyName = SampleSOA.CapabilityScope.Activities[0].Taxons[processTypeIndex].name;
                ...
                for (int processTypeIndex = 0; processTypeIndex < SampleSOA.CapabilityScope.Activities[0].Taxons.Count(); processTypeIndex++)
                {
                    TreeView_Taxonomy tax = new TreeView_Taxonomy(SampleSOA.CapabilityScope.Activities[0].Taxons[processTypeIndex].name);
                    Taxonomies.Add(tax);
